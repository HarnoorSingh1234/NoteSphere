...
export default function ManageSemesters() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || data.message || 'Failed to delete semester');
      }
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between">
              <div>
                <Link 
                  ...
                  className="inline-flex items-center text-[#264143] hover:text-[#DE5499] mb-4 md:mb-0 transition-all duration-200 font-medium"
                >
                ...
                <h1 className="text-3xl font-bold text-[#264143]">Manage Semesters</h1>
              </div>
              <Link 
                href="/admin/semesters/add" 
                className="inline-flex items-center justify-center py-2 px-4 bg-white border-[0.15em] border-[#264143] text-[#264143] font-bold rounded-[0.4em] shadow-[0.1em_0.1em_0_#DE5499] hover:translate-y-[-0.1em] hover:shadow-[0.2em_0.2em_0_#DE5499] transition-all duration-200"
              >
            ...
            <div className="relative z-1 p-6">
              {loading ? (
                <div className="flex items-center justify-center p-12">
                  <div className="w-16 h-16 border-4 border-[#DE5499] border-t-transparent rounded-full animate-spin"></div>
                </div>
              ) : semesters.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <h3 className="text-2xl font-bold text-[#264143] mb-2">No Semesters Found</h3>
                  <p className="text-[#264143]/70 mb-6 max-w-md mx-auto">Get started by adding your first semester</p>
                  <Link 
                    href="/admin/semesters/add" 
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b-[0.2em] border-[#264143]/10">
                        ...
                        <th className="text-left py-4 px-4 text-[#264143] font-bold">Subjects</th>
                        <th className="text-right py-4 px-4 text-[#264143] font-bold">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {semesters.map((semester) => (
                        <tr key={semester.id} className="border-b border-[#EDDCD9] hover:bg-[#F8F5F2] transition-colors">
                          <td className="py-4 px-4">
                            <span className="text-xl font-bold text-[#264143]">Semester {semester.number}</span>
                          </td>
                          <td className="py-4 px-4">
                            <Link 
                              href={`/admin/years/edit/${semester.year.id}`} 
                              className="inline-flex items-center text-[#264143] font-bold px-3 py-1 bg-[#EDDCD9] rounded-[0.3em] border-[0.1em] border-[#264143]/20 hover:bg-[#E99F4C] transition-all duration-200"
                            >
                              Year {semester.year.number}
                            </Link>
                          </td>
                          <td className="py-4 px-4">
                            <span className="inline-block bg-[#EDDCD9] text-[#264143] py-1 px-4 rounded-full font-bold border-[0.1em] border-[#264143]/20">
                              {semester._count?.subjects || 0} {semester._count?.subjects === 1 ? 'Subject' : 'Subjects'}
                            </span>
                          </td>
                          <td className="py-4 px-4 text-right">
                            <div className="flex items-center justify-end space-x-3">
                              <Link 
                                ...
                                className="p-2 bg-[#EDDCD9] border-[0.1em] border-[#264143]/30 text-[#264143] rounded-md hover:bg-[#DE5499] hover:text-white hover:border-[#264143]/50 transition-all duration-200"
                                title="Edit"
                              >
                              ...
                              <button 
                                ...
                                title={deleteId === semester.id ? 'Confirm Delete' : 'Delete'}
                              >
                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                  <path d="M3 6h18"></path>
