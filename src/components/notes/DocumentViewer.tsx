...
/**
 * Document Viewer component that can render various document types from different sources
...
const DocumentViewer: React.FC<DocumentViewerProps> = ({ 
  ...
  const getFileIcon = () => {
    switch(type) {
      case 'PDF':
        return <FileText className="w-5 h-5 mr-2 text-[#ff3e00]" />;
      case 'PPT':
        return <FileText className="w-5 h-5 mr-2 text-[#E99F4C]" />;
      default:
        return <File className="w-5 h-5 mr-2 text-[#4d61ff]" />;
    }
  };
  ...
  useEffect(() => {
    const prepareEmbedUrl = async () => {
      try {
        ...
        if (driveFileId) {
          // Google Drive viewer URL for different file types
          // Google Drive's preview feature works for PDF, DOCX, PPTX, etc.
          ...
          if (fileUrl.includes('drive.google.com')) {
            ...
            // For Office documents, we can try using Microsoft Office Online viewer
            if (type === 'PPT' && !fileUrl.includes('google.com')) {
              setEmbedUrl(`https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(fileUrl)}`);
            } else {
              // For other files, use direct embedding
              setEmbedUrl(fileUrl);
            }
          }
        } else {
        ...
        console.error('Error preparing document for viewing:', err);
    ...
    return (
      <div className={`flex items-center justify-center p-10 bg-[#EDDCD9]/30 border-[0.15em] border-[#264143] rounded-[0.4em] min-h-[400px] ${className}`}>
        <div className="text-center">
          ...
          <h3 className="text-lg font-bold text-[#264143]">Failed to load document</h3>
  ...
  return (
    <div className={`document-viewer-container border-[0.15em] border-[#264143] rounded-[0.4em] overflow-hidden ${className}`}>
      ...
      <iframe 
        ...
        onError={() => {
          console.error(`Failed to load document from URL: ${embedUrl}`);
          setLoading(false);
